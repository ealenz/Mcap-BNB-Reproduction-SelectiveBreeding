...
Temp.Fig <- {plot(Temp~DateTime, data = Temp,ylab = "Temperature (°C)", col = "black", type = "l", lwd = 2, xlab = "Year", ylim = c(20,32)) #Plot NOAA Mokuloe Buoy data from 2014-2016
...
rep16 <- rep1[ which(rep1$Year==2016),] #subset 2016
rep17 <- rep1[ which(rep1$Year==2017),] #subset 2016

rep16mod <- lm(log(microL.cm2) ~ Parental.Performance, rep16)
Anova(rep16mod, type = 2)
summary(rep16mod)
qqnorm(residuals(rep16mod))

rep17mod <- lm(log(microL.cm2) ~ Parental.Performance, rep17)
Anova(rep17mod, type = 2)
summary(rep16mod)
qqnorm(residuals(rep16mod))


repro.mod2 <- lmer(microL.cm2 ~ Parental.Performance + (1|Colony), rep1)
Anova(repro.mod2, type =2)
summary(repro.mod2)
qqnorm(residuals(repro.mod2))
shapiro.test(residuals(repro.mod2))

rep2 <- aggregate(microL.cm2~Parental.Performance + Colony, data = repro, sum, na.rm=TRUE)
hist(rep2$microL.cm2, breaks = 10)

repro.mod3 <- lm(microL.cm2 ~ Parental.Performance, rep2)

Anova(repro.mod3, type = 2)
summary(repro.mod3)
anova(repro.mod3)
qqnorm(residuals(repro.mod3))
shapiro.test(residuals(repro.mod3)) 

leveneTest(residuals(repro.mod1)~rep1$Parental.Performance)
boxplot(residuals(repro.mod1)~rep1$Parental.Performance)
...
annfecsum <- summarySE(rep1, measurevar = "microL.cm2", groupvars = c("Year","Parental.Performance"))
...
```{r}
Vol1 <- summarySE(Vol, measurevar = "vol.mm3", groupvars = c("Year", "SpawningMonth", "Colony", "B.NB","Bundle.ID"), na.rm=T)
range(Vol1$vol.mm3)

Vol2 <- summarySE(Vol1, measurevar = "vol.mm3", groupvars = c("Year", "SpawningMonth", "Colony", "B.NB"), na.rm=T)
range(Vol2$N)

Vol3 <- summarySE(Vol2, measurevar = "vol.mm3", groupvars = c("Year", "SpawningMonth", "Colony", "B.NB"), na.rm=T)

Vol4 <- summarySE(Vol2, measurevar = "vol.mm3", groupvars = c("Year", "B.NB", "Colony"), na.rm=T)
Vol4 <- Vol4[ c(1:4),]
Vol4 #summary
...
Vol.mod <- lmer(vol.mm3 ~ B.NB*Year + (1|SpawningMonth) + (1|Colony),  Vol2)
...
buns1 <- summarySE(buns, measurevar = "EggsPerBundle", groupvars = c("Year", "Colony.ID", "Date", "B.NB"))
buns1 <- buns1[which(buns1$N > 1),]
...
```{r}
hist(buns$EggsPerBundle)
buns.mod1 <- lmer(EggsPerBundle~B.NB*Year + (1|Colony.ID) + (1|SpawningMonth), buns2)
...
```{r}
perab.lm <- lmer((PerAb)~B.NB*Year + (1|Colony.ID) + (1|SpawningMonth), ab.buns1)
...
fert$Prop.2cell <- as.numeric(fert$X2.cell/fert$Fertilized)
fert$Prop.4cell <-  as.numeric(fert$X4.cell/fert$Fertilized)
fert$Prop.8cell <-  as.numeric(fert$X8.cell/fert$Fertilized)
fert$Prop.16cell <-  as.numeric(fert$X16.cell/fert$Fertilized)
...
fert.mod1 <- lmer(perc.fert~Cross + (1|Dam) + (1|Sire), fert)
Anova(fert.mod1, type = 2)
summary(fert.mod1)
emmeans(fert.mod1, pairwise~Cross) #post-hoc analysis - no significant contrasts
...
Anova(fert.mod2)
summary(fert.mod2)
emmeans(fert.mod2, pairwise~Cross, adjust = "Tukey") #marginal significance for 2 contrasts
...
mod.2cell <- lmer(Prop.2cell~Cross + (1|Dam) + (1|Sire), fert)
fixef(mod.2cell)
Anova(mod.2cell, type = 2)
summary(mod.2cell)
...
kruskal.test(Prop.4cell ~ Cross,fert) 
mod.4cell <- lmer(Prop.4cell~Cross + (1|Dam) + (1|Sire), fert)
Anova(mod.4cell, type = 2)
anova(mod.4cell)
summary(mod.4cell)
emmeans(mod.4cell, pairwise~Cross, adjust = "tukey")
qqnorm(residuals(mod.4cell))
plot(residuals(mod.4cell))
shapiro.test(residuals(mod.4cell))


kruskal.test(Prop.8cell ~ Cross,fert) 
mod.8cell <- lmer(Prop.8cell~Cross + (1|Dam) + (1|Sire), fert)
Anova(mod.8cell, type =2)
qqnorm(residuals(mod.8cell))
emmeans(mod.8cell, pairwise~Cross, adjust = "tukey")
plot(residuals(mod.8cell))
emmeans(mod.8cell, pairwise~Cross, adjust = "tukey")
shapiro.test(residuals(mod.8cell))


kruskal.test(Prop.16cell ~ Cross,fert) 
mod.16cell <- lmer((Prop.16cell)~Cross + (1|Dam) + (1|Sire),fert)
Anova(mod.16cell, type = 2)
summary(mod.16cell)
qqnorm(residuals(mod.16cell))
plot(residuals(mod.16cell))
emmeans(mod.16cell, pairwise~Cross, adjust = "tukey")
shapiro.test(residuals(mod.16cell))
...
larv.mod <- lmer(sqrt(Prop.Larvae) ~ Cross + (1|DayPostFert) + (1|EggDonor) + (1|SpermDonor), F1.larv)
Anova(larv.mod, type = 2)
summary(larv.mod)
...
set.mod <- lmer(sqrt(Prop.SettledAlive) ~ Cross  + (1|DayPostFert) + (1|EggDonor) + (1|SpermDonor), F1.set)
Anova(set.mod, type = 2)
summary(set.mod)
posthoc1 <- emmeans(set.mod, pairwise~Cross, adjust = "tukey")
...
mort.mod <- lmer(Dead~Cross + (1|DayPostFert) +(1|EggDonor) + (1|SpermDonor), F1.mort)
Anova(mort.mod, type = 2)
summary(mort.mod)
